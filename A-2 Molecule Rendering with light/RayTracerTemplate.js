function RayTracer(rtCamera,rtContext,rtImgData){
	this.rayTracer=function(moleculeData)
	{
			...
			for (index = 0; index < nSpheres; index++) {
				...
				if (valid) {
					...
					if (tmin<result.tMin){
						...
						result.tMin = tmin;
						result.color = getSphereMaterialColor(spheres,index);
						var pOnRay = pointOnRay(ray, tmin)

						spNormal = sphereNormal(pOnRay, sp).normalize()

						var lightVec = ray.d.clone().negate().normalize()
						var viewVec = ray.d.clone().negate().normalize()
						halfVec = new THREE.Vector3();
						halfVec.addVectors(lightVec.clone(), viewVec.clone()).normalize()
						//console.log(lightVec, viewVec, halfVec);

						var nDotLight = Math.max(0, spNormal.dot(lightVec));  // we do not need the negative value of cosine
						var nDotHalf = Math.max(0, halfVec.dot(spNormal));  // we do not need the negative value of cosine
						//console.log(nDotHalf, nDotLight)

						var materialColorVec = new THREE.Vector3(result.color[0]/255, result.color[1]/255, result.color[2]/255);
						var lightColorVec = new THREE.Vector3(lightColor[0]/255, lightColor[1]/255, lightColor[2]/255);
						

						var diffuseColor = materialColorVec.clone().multiply(lightColorVec.clone());
						var diffuseColor = diffuseColor.clone().multiplyScalar(nDotLight);

						var specularColor = lightColorVec.clone().multiplyScalar( Math.pow(nDotHalf, shininess ));

						reflectedColor = new THREE.Vector3();
						var reflectedColor = reflectedColor.addVectors( diffuseColor.clone().multiplyScalar(1-Ks), 
																		specularColor.clone().multiplyScalar(Ks) );
						reflectedColor.clampScalar(0, 1);
						result.color = [reflectedColor.x * 255, reflectedColor.y * 255, reflectedColor.z * 255]




						// var diffuseColor = result.color.map(function(x, i) { return x * lightColor[i] * nDotLight });
						// var specularColor = lightColor.map(function(x) { return x * Math.pow(nDotHalf, shininess) });

						// reflectedColor = diffuseColor.map(function(x, i) { return (x*(1-Ks)) + (Ks * specularColor[i]) });
						// result.color = reflectedColor.map(function(x) { return  Math.min( Math.max(0, x), 255)});    // clamping between 0 and 255
		...
		function pointOnRay(ray, t) {
			...
			origin = ray.O;
			dirVec = ray.d;
			p = origin.clone().add(dirVec.clone().multiplyScalar(t));
